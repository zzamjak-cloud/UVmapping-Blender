...
8. `다면도 구성`에서 `6면도 · 3:2`(기본값, FRONT/RIGHT/BACK/LEFT/TOP/BOTTOM 3×2)와 `3면도 · 21:9`(FRONT/RIGHT/BACK 3열) 중 하나를 고르고 `생성 이미지 크기`(자동/1K/2K/4K)를 정합니다. 기본값 `자동`은 6면도 2K(시점당 512px), 3면도 1K를 씁니다. 어느 쪽이든 OpenRouter 호출은 한 번입니다. 6면도는 상·하·좌 시점을 실제 그림으로 받아 투영하므로 머리 위, 발바닥, 비대칭 왼쪽 면이 정확해집니다. 시점당 해상도를 두 배로 올리고 싶으면 4K(6면도 시점당 1024px)를 직접 고르되, 호출 비용이 그만큼 커집니다.
8-2. `실루엣 불일치 시 자동 재생성`(기본 1회)은 생성 직후 시점별 그림의 실루엣을 모델 실루엣과 대조해, 팔을 몸에 붙여 그리는 식으로 내부 구조가 달라진 시점이 있으면 "가이드에서 배경이 보이는 틈은 그대로 남기고 회색 영역은 빠짐없이 채색"하라는 교정 문단을 붙여 다시 생성합니다. 재생성마다 호출이 추가되며, 0으로 두면 경고만 남기고 진행합니다.
...
10. `단일 다면도 생성`을 누르면 대상 모델을 선택한 시점 방향으로 로컬 캡처해 흰 배경 위 회색 형상 가이드를 격자로 만든 뒤 OpenRouter를 한 번만 호출합니다. AI는 이 가이드의 실루엣, 비율, 자세를 그대로 두고 그 위에 색만 입히도록(paint-over) 지시받습니다. 종횡비는 구성에 따라 3:2 또는 21:9로 고정되고 결과는 한 장입니다.
...
현재 CPU 베이크는 최대 4096×4096을 지원하며 기본값은 1024×1024입니다. `텍스처 크기`와 `UV 패딩`은 `.blend`에 저장되므로, 이전에 저장한 파일은 그때 값이 그대로 유지됩니다. 3면도 구성에서는 직접 보이지 않는 상·하면을 인접 시점의 색을 확장해 채우므로, 의미 있는 상면 그림이 필요한 에셋은 6면도 구성을 사용합니다. 다면도 생성 시점과 베이크 시점에 각각 현재 UV로 계약을 다시 계산해 비교하므로, 모델 Transform, 메시 또는 UV가 그 사이에 바뀌면 잘못 투영하지 않고 재생성을 안내합니다.
