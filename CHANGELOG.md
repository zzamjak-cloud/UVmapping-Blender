...
- **생성 실패 원인 팝업**: AI 생성이 실패하면 원문을 분류해 한국어 제목·설명·해결 방법을 팝업으로 보여 주고, 패널 상태 상자 아래에도 `실패 원인 보기` 버튼을 남깁니다. 분류는 콘텐츠 정책 거부, API 키 인증, 요청 한도, 요청 형식, 네트워크, 이미지 없음, 서버 오류, 애드온 내부 실패, 알 수 없음 9가지입니다.
- 콘텐츠 정책 거부 안내는 유명 캐릭터·브랜드·실존 인물 이름을 빼고 외형으로 묘사하기, 공식 아트 대신 자체 원화 참조하기, 모델을 바꿔도 같은 입력이면 다시 막힌다는 점을 함께 알립니다.
- 제공사 문의에 필요한 요청 ID(`req_…`)를 따로 표시하고, `원문 복사` 버튼으로 실패 원문 전체를 클립보드에 복사합니다.
- **전문가 설정 2종 추가**: `시점 경계 선명하게`(기본 켬)와 `경계 혼합 폭(도)`(기본 12, 범위 4~30). 띠 폭은 지배 시점 선택이 켜져 있을 때만 조절할 수 있고, 두 값은 상태의 `bake_settings`에 기록돼 재적용에도 같은 조건으로 굽습니다.

### 변경

- **투영 블렌딩 개선**: 픽셀마다 가장 정면으로 보이는 지배 시점을 고르고, 그 시점과 입사각 차가 전이 띠 안인 시점만 함께 섞습니다. 정합이 어긋난 두 시점이 겹쳐 보이던 경계 이중상과 상·하면 얼룩이 줄어듭니다.
- 실루엣 밖으로 확장한 표본은 신뢰도를 제곱으로 낮추고, 확장 한계를 실루엣 폭의 3%에서 1.5%로 좁혔습니다. 정면 색이 측면으로 번지는 현상이 줄어듭니다.
- 모든 실패 지점(작업자 응답 실패, 그룹 실패, 참조 분석 실패, 재생성 실패, 자동 적용 실패, 순차 생성 실패)이 공통 기록부를 거쳐 원문과 분류를 함께 남깁니다. 상태 줄 문구 형식은 그대로입니다.
- 이미지 없이 글로만 돌아온 응답은 모델이 남긴 거부 사유를 300자까지 오류 메시지에 함께 싣습니다.

## 2.5.0 - 2026-09-21

### 추가

- **품질 프리셋**(`품질`, 기본 `표준`): `빠름`(3면도 1회·1K·다시 그리기 끔·검증 끔)/`표준`(6면도 1회·1K·다시 그리기 1회·검증 켬)/`고품질`(QUAD 4회·2K·다시 그리기 1회·검증 켬)/`사용자 지정` 중 하나를 고르면 시점 구성·생성 이미지 크기·텍스처 크기·다시 그리기 횟수·적용 후 검증·품질 단계를 한 번에 맞춥니다. 고급 설정에서 개별 값을 직접 바꾸면 `사용자 지정`으로 전환됩니다.
- **텍스처 생성 버튼 체인**: 참조 이미지가 있고 분석 결과가 없거나 참조가 바뀌었으면 분석을 먼저 실행하고 이어서 다면도 생성, `생성 후 자동 적용`이 켜져 있으면 Diffuse 적용까지 버튼 하나로 이어집니다. 기존 `참조 이미지 분석`·`Diffuse/Albedo 적용` 버튼은 고급 설정에 남아 있습니다.
- **모델 프리셋 2종 추가**: `GPT Image 2.5 Sunburst (정밀, 느리고 비쌈)`와 `GPT Image 2.5 Flare (속도 우선)`(둘 다 "덕테이프 2.5" 별칭 병기). 기본값은 `Nano Banana Pro`로 유지합니다.
- 이미지 모델별 능력치 표(`openrouter_provider.IMAGE_MODEL_CAPABILITIES`)를 두어 종횡비·해상도 지원 여부·품질 등급·참조 상한을 모델마다 검사합니다.
- **품질 단계**(`texture_image_quality`, 기본 자동): 해상도 대신 품질 등급을 받는 GPT 계열 모델에 보낼 `medium`/`high`/`xhigh` 등급입니다. 모델이 지원하지 않는 등급은 자동으로 한 단계씩 낮춰 보냅니다.

### 변경

- N패널을 대상 → 참조·지시 → 품질/모델 → 텍스처 생성 순의 기본 화면으로 재구성하고, 세부 옵션은 `고급 설정` 접힘 안으로, 전문가용 3개(시점 전이 폭·색조 보정·행 워프)와 모델 슬러그 직접 입력은 그 안의 `전문가 설정` 하위 접힘으로 옮겼습니다.
- GPT 계열 모델은 `resolution` 대신 `quality`를 보내며, 결과 이미지 크기가 종횡비로 1K급에 고정됩니다. `build_turnaround_payload`가 모델이 실제로 받는 파라미터만 전송합니다.
- `텍스처 크기` 선택지에서 8192 px을 제거했습니다(베이크 상한 4096과 일치).
- `다면도 구성` → `시점 구성`, `실루엣 불일치 시 자동 재생성` → `형태가 어긋나면 다시 그리기(횟수)`로 라벨을 정리했습니다.
- 정상 상태일 때 표시하던 "OpenRouter API 키 사용 가능" 라벨을 제거하고, 문제(온라인 접근 차단·API 키 미입력·UV 누락)가 있을 때만 경고 상자를 보여 줍니다.

### 유지

- 기존 씬에 저장된 개별 옵션 값은 패널을 처음 열 때 덮어쓰지 않습니다. 저장된 값이 어느 프리셋 표와도 다르면 `품질`은 `사용자 지정`으로 표시됩니다.
- `NANO_BANANA_PRO`/`GPT_IMAGE` 모델 프리셋의 슬러그와 라벨, 기존 상태 스키마는 그대로입니다.

## 2.4.0 - 2026-09-21

### 추가

- **QUAD 구성**(`다면도 구성` → `6면도 · 4회 호출`): 같은 6시점을 정면 단독·뒷면 단독·좌우 2칸·상하 2칸 캔버스 4장으로 나눠 받는다. 정면을 먼저 생성해 그 결과를 색 기준으로 삼고 나머지 세 그룹을 병렬 호출하는 2라운드로 진행한다(호출 4회).
- 워커의 그룹 배치 병렬 호출과 429/5xx 지수 백오프 재시도(최대 3회).
- 실루엣 불일치 시 QUAD는 시점이 아니라 그룹 단위로만 재생성한다.

### 변경

- 텍스처 상태 스키마 1.4: `composition`·`groups` 필드 추가(그룹 구성만, 기존 필드는 유지).
- 패널의 호출 수 경고를 구성 공통 문구로 일반화하고, QUAD의 2라운드 진행을 안내 문구로 표시한다.
- `생성 이미지 크기` 안내 문구를 절대 픽셀 수치 대신 상대 비교(캔버스가 적을수록 시점당 해상도가 커짐)로 교정했다.

### 유지

- 3면도·6면도(3:2)·순차 인페인팅의 호출 수, 프롬프트, 결과, 상태 파일은 2.3.0과 동일하다. 구버전 상태(스키마 1.3)도 그대로 베이크된다.

## 2.3.0 - 2026-09-12

### 추가

- **6면도 구성**(기본값): FRONT/RIGHT/BACK/LEFT/TOP/BOTTOM을 3×2 격자 한 장(3:2)으로 생성해 호출 1회를 유지하면서 머리 위·발바닥·비대칭 왼쪽 면을 실제 그림으로 투영한다. 기존 3면도(21:9)는 `다면도 구성`에서 선택할 수 있고 구버전 3면도 상태도 그대로 베이크된다.
- **생성 이미지 크기** 설정: 기본 `자동`은 6면도 2K(시점당 512px), 3면도 1K를 쓰고 4K는 직접 선택한다.
- **순차 인페인팅 생성 방식**: 시점마다 한 번씩 호출해 앞 시점을 투영한 모델을 다음 시점의 가이드로 보낸다. 비용이 시점 수만큼 늘어나므로 UI에 호출 횟수를 표시한다. 중단된 실행은 `생성 상태 초기화`로 정리한다.
- **매핑 검증 루프**: 생성 직후 시점별 실루엣 불일치(행 구간 수 불일치·밝은 구멍)를 검사해 `자동 재생성` 횟수 안에서 교정 프롬프트로 다시 생성하고, 베이크 후에는 텍스처가 입혀진 모델을 시점별로 렌더해 생성 이미지와 비교한 점수와 검증 시트(`*_verify.png`: 가이드/생성/렌더)를 패널 `검증` 상자에 표시한다.
- 투영 블렌딩 설정: `뷰 전이 지수`, `뷰 간 색 보정`, `실루엣 행 워프(실험적, 기본 꺼짐)`.

### 수정 (투영 정확도)

- 뷰 가중치를 면 단위 cos²에서 **정점 법선 보간 픽셀 단위 cos⁴**로 바꾸고, 가려짐 판정을 불린에서 깊이 차 기반 soft 가중으로, 실루엣 밖 표본을 하드 컷오프에서 거리 감쇠로 바꿔 시점 경계의 계단 이음매를 없앴다.
- 모델 실루엣(깊이 버퍼)을 생성 이미지에 IoU 기준으로 정합해 배경 판정 마스크로 쓰고, 배경보다 밝은 무채색 픽셀과 테두리 연결 영역을 배경으로 확정해 팔·몸통 틈이나 발 사이의 순백이 텍스처에 묻어나던 문제를 해결했다.
- 시점 간 색 보정을 휘도 스칼라 하나로 제한하고 FRONT와 직접 겹치는 표본만 쓰며 분산이 크면 보정하지 않는다. 이전에는 정합 오차 지점의 표본이 체인 전파되어 1.4배까지 밝아지며 흰색으로 포화됐다.
- 실루엣 밖 색 확장 거리를 6% → 3%로 줄여 팔 가장자리의 가로 줄무늬를 완화했다.

### 변경

- 연산자 라벨 `단일 3면도 생성` → `단일 다면도 생성`. 텍스처 상태 스키마 1.3(`layout`, `image_size`, `bake_settings`, `silhouette_report`, `verification`).
- 프롬프트 형상 계약 첫 항목을 "새 그림이 아니라 가이드 위 픽셀 정렬 채색(paint-over)"으로 강화했다.
